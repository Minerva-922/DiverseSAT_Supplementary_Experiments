"""
CPLEX DW (Decreasing Weights, one-hot) encoding + SEv3 standalone.

Linear objective with the one-hot U_{j,r} counting indicator, plus
SEv3 (see ``materials/14-symmetry_elimination.pdf`` Version 3) as
the sole symmetry-breaking block.  SEv1 is NOT added.
"""
...
def create_variables(prob: cplex.Cplex, n: int, k: int):
    ...
    prob.variables.add(
        names=[f"U_{j}_{r}" for j in range(n) for r in range(0, k+1)],
        types=[prob.variables.type.binary] * (n*(k+1)),
        obj=[r*(k-r) for j in range(n) for r in range(0, k+1)]
    )
...
def add_counting_constraints(prob: cplex.Cplex, n: int, k: int):
    for j in range(n):
        ...
        u_vars = [f"U_{j}_{r}" for r in range(0, k+1)]
        ...
        prob.linear_constraints.add(
            lin_expr=[cplex.SparsePair(u_vars, [1] * len(u_vars))],
            senses=["E"],
            rhs=[1]
        )
