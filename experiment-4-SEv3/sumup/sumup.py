...
DIR_PATTERN = re.compile(r'^result-(.+?)-k_(\d+)-([A-Z]+)-((?:SE|no).*?)-(\d{14})$')
...
def get_roundingSAT_record(filename, filepath):
    ...
    for line in lines:
        ...
        if line.startswith('o '):
            ...
            try:
                last_bounds_val = int(line.split()[2])
    ...
    if OPT is None and is_BEST == 1 and last_bounds_val is not None:
        OPT = -last_bounds_val
