#!/usr/bin/env python3
"""
Generate SLURM RoundingSAT solve-job scripts for experiment-4.

Assumes the OPB files produced by ``../transform/`` already live under
``../benchmarks/k_<k>-<enc>-SEv3/``.
"""
...
def job_name(k, encode, se, solver):
    ...
    return f"{k}{enc_map.get(encode, encode[0])}v3R"
...
def main():
    ...
    with open(submit, "w") as f:
        f.write("#!/bin/bash\n")
        f.write("# experiment-4 RoundingSAT submission (SEv1 + SEv3 redundant)\n")
