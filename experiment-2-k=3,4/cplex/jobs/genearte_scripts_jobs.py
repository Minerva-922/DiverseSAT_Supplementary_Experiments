...
experiment-2 scope:
    k in {3, 4} — complementary to experiment-1 (k in {2, 5, 10}).
    Per the meeting decision, we keep the FEWEST_TESTS layout: SEv1 is treated as
    the new baseline, so QP / DW / IW only run with SE; BIN still runs with SE
    and noSE for the ablation.
"""
...
# Configuration
#
# NB. As Sami noted in the JAIR meeting (2026-03, 22:41): once uniqueness
# is baked into the problem definition, SE vs noSE are no longer two
# valid ablation points — the noSE version does not even solve the same
# problem. We therefore drop noSE entirely and run every encoding with
# the new SEv1 baseline.
k_values = [3, 4]
SE_modes = ["SE"]
encodes = ["QP", "DW", "IW", "BIN"]
solvers = ["CPLEX"]

# Kept as a no-op to preserve the call sites below; always false now.
FEWEST_TESTS = False
...
def should_skip_combination(encode, SE_mode):
    # Everything gets generated; noSE no longer part of SE_modes.
    return False
