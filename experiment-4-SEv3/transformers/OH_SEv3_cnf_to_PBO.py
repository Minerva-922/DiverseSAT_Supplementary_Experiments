...
**SEv3-alone** symmetry-elimination scheme.

SEv3 (per materials/14-symmetry_elimination.pdf, Version 3):
    V_{j,j}  ->  V_{i,j}     for j in {1, ..., K-1}, i in {j+1, ..., K}

Produces K*(K-1)/2 binary clauses, no auxiliary variables.  See
``_pbo_io.add_SEv3_diagonal`` for the full definition + proof sketch.

This configuration intentionally DOES NOT apply SEv1 — it tests SEv3
as a standalone (partial) symmetry breaker.

Uniqueness: following the exp-1 / exp-2 convention, no explicit
distinctness constraint is enforced here either.  Pairwise distinctness
emerges implicitly through the max-diversity objective (duplicate
copies contribute 0 Hamming).
"""
...
from _pbo_io import PBOWriter, add_SEv3_diagonal  # noqa: E402
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="python OH_SEv3_cnf_to_PBO.py <in.cnf> <out.opb> <K>"
    )
