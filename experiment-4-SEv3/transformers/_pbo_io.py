...
def add_SEv3_diagonal(pbo: PBOWriter, V, K: int) -> int:
    ...
    In experiment-4 we use this block **standalone** (no SEv1) — it
    fully replaces the SEv1 chain.  A combined "SEv1 + SEv3 redundant"
    mode was considered but dropped, because SEv1's unique lex-minimal
    permutation and SEv3's diagonal-minimal permutation need not
    coincide, so conjoining them as hard constraints can make the
    problem UNSAT.
